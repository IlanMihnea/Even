...
function editProperty(id, cat) {
  showToast(`Editare ${id} — deschide formularul cu datele existente.`);
}
...
async function submitNewProperty(e) {
  ...
  formData.id = `${activeFormCategory.slice(0,3)}-${Date.now()}`;

  try {
    await upsertProperty(activeFormCategory, formData);
    closeAddModal();
    showToast(`Proprietate adăugată în ${capitalize(activeFormCategory)}!`);
